  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    await new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        }, 3000); // Reduced from 5000ms to 1000ms (1 second)
      } catch (error) {
        console.error(`Chunk ${i + 1} error sending bundle:`, error);
        reject(error);
      }
...
export async function sendBundlesForCreate(
  ...
  await new Promise(async (resolve, reject) => {
      ...
      const onSuccess = (bundleResult: BundleResult) => {
          ...
          if (bundleResult.rejected?.droppedBundle?.msg?.includes('Bundle partially processed') ||
              bundleResult.rejected?.droppedBundle?.msg?.includes('transaction already processed') ||
              (typeof bundleResult.rejected?.droppedBundle?.msg === 'string' && 
               bundleResult.rejected.droppedBundle.msg.toLowerCase().includes('already processed'))) {
              console.log(`Bundle partially processed or transaction already processed - considering as success`);
              resolve(undefined);
              return;
          }
          ...
          if (bundleResult.rejected) {
              ...
              // Check if any of the error messages indicate "already processed"
              if (JSON.stringify(bundleResult).toLowerCase().includes('already processed')) {
                  console.log('Transaction already processed - considering as success');
                  resolve(undefined);
                  return;
              }
              reject(new Error(`Bundle rejected: ${JSON.stringify(bundleResult)}`));
      ...
      const onError = (error: any) => {
          ...
          // Check if error message contains "already processed"
          if (error.message && error.message.toLowerCase().includes('already processed')) {
              console.log('Transaction already processed error - considering as success');
              resolve(undefined);
              return;
          }
      ...
      try {
          try {
            ...
            console.log('Continuing with bundle sending despite listener error');
          }
          ...
          // Check if sending error contains "already processed"
          if (error.message && error.message.toLowerCase().includes('already processed')) {
              console.log('Transaction already processed error during send - considering as success');
              resolve(undefined);
              return;
          }
